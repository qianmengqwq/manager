...
export async function addMajor(params: AddMajorParams) {
  try {
    const response = await fetch(`/api/college/addmajor`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
// 更新专业
export async function updateMajor(params: UpdateMajorParams) {
  try {
    const response = await fetch(`/api/college/updatemajor`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (data.code !== 1000) {
      throw new Error(data.msg || '更新专业失败')
    }

    return data.result
  }
  catch (error) {
    console.error({ updateMajorError: error })
    throw error
  }
}
...
export async function deleteMajor(majorid: number) {
  try {
    const response = await fetch(`/api/college/deletemajor`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
