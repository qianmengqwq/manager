...
function ActivityDetailPage() {
  ...
  // 定义注册表格列
  const registrationColumns = useMemo<ColumnDef<(typeof mockRegistrations)[0]>[]>(() => [
    {
      id: 'studentName',
      accessorKey: 'studentName',
      header: ({ column }) => <DataTableColumnHeader column={column} title="学生姓名" />,
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="所属院系" />,
      enableColumnFilter: true,
      meta: {
        label: '所属院系',
      },
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm text-muted-foreground">
              通过率
              {' '}
              {Math.round(activity.registrationCount.approved / activity.registrationCount.total * 100)}
              %
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm text-muted-foreground">
              拒绝率
              {' '}
              {Math.round(activity.registrationCount.rejected / activity.registrationCount.total * 100)}
              %
            </div>
          </CardContent>
        </Card>
